...
shapley value는 이러한 문제를 극복하기 위해서 개별 팀원의 모든 경우의 수를 모두 고려해서 개별 팀원의 기여도를 계산하는 방식을 채택한다.

![<https://www.youtube.com/watch?v=9haIOplEIGM&t=152s>](image12.png)

머신러닝 모델로 일반화를 하면 팀 내의 개별 팀원은 feature로 볼 수 있고, Game은 Black box 모델, Payout은 prediction 값으로 볼 수 있다. 정리하면 shapley value는 각 feature가 예측에 얼마나 기여를 했는지 측정하는 측도로 볼 수 있고, 이 측도를 구하기 위해서 모든 하위 부분집합의 값을 계산하는 방법을 채택한다.
...
아래 이미지의 예시는 X1 변수에 대해 shapley value를 구하는 예제이다.

먼저 X1 변수를 제외할 수 있는 경우의 수에 따라 각 쌍을 구성하고 예측값의 차이를 계산한다.

![<https://www.youtube.com/watch?v=f2XqxOny3NA&t=299s>](image13.png)

두번째로, 변수 하나만 사용했을 때, 변수를 2개 사용했을 때, 변수를 3개 사용했을 때의 쌍과 X1이 제외될 수 있는 쌍 사이의 비율을 이용해서 가중치를 구한다.

![<https://www.youtube.com/watch?v=f2XqxOny3NA&t=299s>](image14.png)

마지막으로, 첫번째 단계에서 구한 예측값의 차이에 두번째 단계에서 구한 가중치를 이용한 가중합을 이용해서 shapley value를 구한다.

![<https://www.youtube.com/watch?v=f2XqxOny3NA&t=299s>](image15.png)
...
xgb_fit <- xgb_wflow %>% fit(data = ames_train)

xgb_ex_fit <- xgb_fit %>% extract_fit_parsnip()
